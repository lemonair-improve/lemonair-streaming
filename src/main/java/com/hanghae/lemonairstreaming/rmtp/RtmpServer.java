...
public abstract class RtmpServer implements CommandLineRunner {
	...
	private void sendStreamingIsOnAirToServiceServer(Stream stream, Long s) {
		...
		log.info(serviceServerIp + "/api/streams/" + stream.getStreamerId() + "/streaming");
		webClient.post() // 비동기 post 요청
			.uri(serviceServerIp+":"+serviceServerPort + "/api/streams/" + stream.getStreamerId() + "/streaming") // post 요청 uri (컨텐츠 서버)
			.retrieve() // 응답 수신
