		...
		DisposableServer server = TcpServer.create() // TCP 서버 생성
			...
			.handle((in, out) -> in
				...
				.flatMap(stream -> { // 각 스트림 객체에 대한 연산
					...
					return Mono.empty(); // 스트리밍 시작시에 클라이언트 <-> rtmp 서버 연결관계 확인 후 return된다.
				})
				.then())
			.bindNow(); // 서버의 환경 설정과 구독 관계 설정이 끝나면 서버가 BindNow된다.
		server.onDispose().block(); // disposableServer는 종료될 수 있는 서버 같은데 onDispose().block()을 걸어서 종료되지 않도록 막는다.
	}
